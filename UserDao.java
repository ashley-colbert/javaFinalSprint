...
public class UserDao {
    ...
    public boolean createUser(User user) {
        ...
        String user_query = "INSERT INTO users (first_name, last_name, email, password, is_doctor) VALUES (?, ?, ?, ?, ?)";

        // Database logic to insert data using PREPARED Statement
        try {
            Connection con = DatabaseConnection.getCon();
            PreparedStatement statement = con.prepareStatement(user_query);
    ...
    public User getUserById(int id) { //get user by id from database
        ...
        String id_query = "SELECT * FROM users WHERE id = ?";
    ...
    public User getUserByEmail(String email) { // get user by email from database
        ...
        try {
            Connection con = DatabaseConnection.getCon();
            PreparedStatement statement = con.prepareStatement(email_query);
    ...
    public boolean updateUser(User user) {
        ...
        "SET first_name = ?, last_name = ?, email = ?, password = ?, is_doctor = ? WHERE id = ?";
        String hashedPassword = BCrypt.hashpw(user.getPassword(), BCrypt.gensalt());
    ...
    // SQL Statement
        String pw_query = "SELECT password FROM users WHERE email = ?";
    
    //Implement logic to retrieve password using the Bcrypt
    try {
        Connection con = DatabaseConnection.getCon();
        PreparedStatement statement = con.prepareStatement(pw_query);
        statement.setString(1, email);
        ResultSet rs = statement.executeQuery();
